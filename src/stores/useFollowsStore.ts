...
export const useFollowsStore = defineStore('useFollowsStore', () => {
    ...
    const fetchPendingFollowRequests = async () => {
        ...
        const response = await axios.get('http://66.42.81.246/api/v1/pending-follow-requests', {
            headers: {
    ...
    const fetchFollowers = async () => {
        ...
        const response = await axios.get('http://66.42.81.246/api/v1/followers', {
            headers: {
                'x-auth-token': getAuthToken.value
            }
        });
    ...
    const fetchFollowings = async () => {
        ...
        const response = await axios.get('http://66.42.81.246/api/v1/followings', {
            headers: {
                'x-auth-token': getAuthToken.value
            }
        });
    ...
    const acceptDenyRequest = async (action: 'accept' | 'deny', followerUserId: number) => {
        ...
        await axios.patch('http://66.42.81.246/api/v1/followers', {
            action, 
